...
fprintf('\n=== Augment_2linear_hyper.m Linear SVR 미실험 조건 예측 (하이퍼파라미터 최적화 + K-fold CV) ===\n');
fprintf('데이터 로딩 중...\n');
...
fprintf('\n=== Linear SVR 하이퍼파라미터 최적화 및 K-fold CV 시작 ===\n');
...
fprintf('\nLinear SVR 하이퍼파라미터 최적화 완료!\n');

%% 4. 최적화된 Linear SVR 모델 학습 및 예측
fprintf('\n=== 최적화된 Linear SVR 모델 학습 및 예측 시작 ===\n');
Y_predict_norm = zeros(size(X_predict,1), num_outputs);

for j = 1:num_outputs
    fprintf('출력변수 %s (%d/%d) 최적화된 Linear 예측 중...\n', output_names{j}, j, num_outputs);
    Yt_train = Y_train_norm(:,j);
...
% 6-1. 성능 지표 막대그래프
figure('Name','Linear SVR K-fold CV 성능 지표','WindowStyle','docked');
...
sgtitle('최적화된 Linear SVR K-fold 교차검증 성능 지표', 'FontSize', 14, 'FontWeight', 'bold');
...
% 7-1. 박스플롯: Linear 예측 vs 실제
fprintf('박스플롯 생성 중...\n');
figure('Name','최적화된 Linear 예측 분포 Boxplot: 미실험(112) vs 실제(16)','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
    data_box = [Y_train(:,j); Y_predict(:,j)];
    labels_box = [repmat({'실제 (16)'},size(Y_train,1),1); repmat({'Linear 예측 (112)'},size(Y_predict,1),1)];
    boxplot(data_box, labels_box);
    title(['Output ',output_names{j}]); ylabel(output_names{j}); grid on;
end
sgtitle('최적화된 Linear SVR: 실제(16) vs 미실험(112) 예측 분포비교','FontSize',14,'FontWeight','bold');

% 7-2. 산점도: Linear 예측조건별 실제값범위와 예측값
fprintf('산점도 및 히스토그램 생성 중...\n');
figure('Name','최적화된 Linear 예측 산점도 및 히스토그램: 조건별 예측값 분포','WindowStyle','docked');
for j = 1:num_outputs
    subplot(4,2,j);
    scatter(1:size(Y_predict,1), Y_predict(:,j), 60, 'g', 'filled'); hold on;
    yline(min(Y_train(:,j)),'k:','최소실제'); yline(max(Y_train(:,j)),'k:','최대실제');
    grid on; ylabel(output_names{j}); legend('최적화 Linear SVR','location','best');
    title(['미실험 112조건 ',output_names{j},' 최적화 Linear 예측분포']);
    ...
    grid on; xlabel(output_names{j}); ylabel('count'); legend('최적화 Linear SVR','location','best');
    title(['미실험 112조건 ',output_names{j},' 최적화 Linear 예측 히스토그램']);
end
sgtitle('최적화된 Linear SVR 조건별 출력 예측 산점도 및 분포(16실제값 참조선)','FontSize',14,'FontWeight','bold');

% 7-3. 히트맵 시각화(최적화된 Linear)
fprintf('히트맵 생성 중...\n');
figure('Name','최적화된 Linear 예측값 히트맵','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
    Ymat = reshape(Y_predict(:,j), [14, 8]);
    imagesc(Ymat); colorbar;
    title(['Output ',output_names{j},' 최적화 Linear 예측 히트맵']);
    xlabel('X3*X4 조건 인덱스'); ylabel('X1*X2 조건 인덱스');
end
sgtitle('최적화된 Linear SVR 기반 예측값 히트맵','FontSize',14);

%% 8. 결과 저장
fprintf('\n=== 결과 저장 ===\n');

% 최적화된 Linear 예측값 저장
fprintf('최적화된 Linear SVR 예측값을 predict_linear_hyper.csv로 저장 중...\n');
...
fprintf('저장 완료: predict_linear_hyper.csv (%d개 예측조건)\n', size(X_predict,1));

% 최적 하이퍼파라미터 저장
fprintf('최적 하이퍼파라미터를 hyperparams_Augment_2linear.mat로 저장 중...\n');
save('hyperparams_Augment_2linear.mat', 'best_params', 'CV_scores', 'rmse_scores', 'mae_scores', ...
     'output_names');
fprintf('저장 완료: hyperparams_Augment_2linear.mat\n');

fprintf('\n=== Augment_2linear_hyper.m 실행 완료 ===\n');
fprintf('주요 개선사항:\n');
