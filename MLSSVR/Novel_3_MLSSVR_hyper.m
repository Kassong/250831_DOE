...
fprintf('\n=== 개별 MLSSVR 모델 학습/평가 시작 (확장된 하이퍼파라미터) ===\n');
fprintf('학습데이터: predict_MLSSVR.csv, 테스트데이터: dataset.csv\n');
T_train = readtable('predict_MLSSVR.csv');      % 112개
...
%% 확장된 하이퍼파라미터 그리드 서치 함수 정의
function [best_gamma, best_lambda, best_p, best_mse] = GridMLSSVR_Extended(X, Y, cv_folds)
    % 확장된 하이퍼파라미터 범위
    ...
    fprintf('확장된 하이퍼파라미터 탐색 시작: 총 %d개 조합\n', total_combinations);
                ...
                for fold = 1:cv_folds
                    ...
                    try
                        ...
                        if size(Y_fold_test, 2) == 1
                            fold_mse = mean((Y_fold_test - Y_pred).^2);
                        else
...
%% 각 출력 변수별로 개별 모델 학습 및 평가 루프
for j = 1:num_outputs
    fprintf('\n====== 출력 변수 %s (%d/%d) 모델링 시작 (확장된 하이퍼파라미터) ======\n', output_names{j}, j, num_outputs);

    % 현재 출력 변수에 해당하는 데이터만 선택
    Y_train_norm_single = Y_train_norm(:, j);
    Y_test_norm_single  = Y_test_norm(:, j);

    % --- 10-fold 교차검증 (개별 모델) ---
    fprintf('10-fold 교차검증 시작...\n');
    k_folds = 10;
    n_train = size(X_train_norm, 1);

    % 확장된 하이퍼파라미터 서치 (현재 출력 변수 데이터만 사용)
    [gamma_opt, lambda_opt, p_opt, mse_opt] = GridMLSSVR_Extended(X_train_norm, Y_train_norm_single, 5);
    fprintf('최적 파라미터 (MSE=%.6f): gamma=%.4f, lambda=%.4f, p=%.4f\n', mse_opt, gamma_opt, lambda_opt, p_opt);

    cv_indices = cvpartition(n_train, 'KFold', k_folds);
    cv_pred = zeros(n_train, 1);
    for fold = 1:k_folds
        train_idx = training(cv_indices, fold);
        test_idx = test(cv_indices, fold);
        
        X_cv_train = X_train_norm(train_idx, :);
        Y_cv_train_single = Y_train_norm_single(train_idx, :);
        X_cv_test = X_train_norm(test_idx, :);
        
        [alpha_cv, b_cv] = MLSSVRTrain(X_cv_train, Y_cv_train_single, gamma_opt, lambda_opt, p_opt);
        
        Y_cv_test_dummy = zeros(sum(test_idx), 1);
        [pred_cv, ~, ~] = MLSSVRPredict(X_cv_test, Y_cv_test_dummy, X_cv_train, alpha_cv, b_cv, lambda_opt, p_opt);
        cv_pred(test_idx) = pred_cv;
    end
    
    SS_res_cv = sum((Y_train_norm_single - cv_pred).^2);
    SS_tot_cv = sum((Y_train_norm_single - mean(Y_train_norm_single)).^2);
    cv_scores(j) = 1 - SS_res_cv / SS_tot_cv;
    fprintf('10-fold CV R² = %.4f\n', cv_scores(j));

    % --- 최종 개별 모델 학습 및 예측 ---
    fprintf('최종 모델 학습 및 예측...\n');
    [alpha, b] = MLSSVRTrain(X_train_norm, Y_train_norm_single, gamma_opt, lambda_opt, p_opt);

    Y_train_dummy = zeros(size(Y_train_norm_single));
    [Y_pred_train_norm(:, j), ~, ~] = MLSSVRPredict(X_train_norm, Y_train_dummy, X_train_norm, alpha, b, lambda_opt, p_opt);
    
    Y_test_dummy = zeros(size(Y_test_norm_single));
    [Y_pred_test_norm(:, j), ~, ~] = MLSSVRPredict(X_test_norm, Y_test_dummy, X_train_norm, alpha, b, lambda_opt, p_opt);

    % --- 성능 지표 계산 ---
    RMSE_train(j) = sqrt(mean((Y_train_norm_single - Y_pred_train_norm(:, j)).^2));
    RMSE_test(j)  = sqrt(mean((Y_test_norm_single - Y_pred_test_norm(:, j)).^2));
    MAE_train(j)  = mean(abs(Y_train_norm_single - Y_pred_train_norm(:, j)));
    MAE_test(j)   = mean(abs(Y_test_norm_single - Y_pred_test_norm(:, j)));
    
    SS_res_train = sum((Y_train_norm_single - Y_pred_train_norm(:, j)).^2);
    SS_tot_train = sum((Y_train_norm_single - mean(Y_train_norm_single)).^2);
    R2_train(j) = 1 - SS_res_train/SS_tot_train;
    
    SS_res_test = sum((Y_test_norm_single - Y_pred_test_norm(:, j)).^2);
    SS_tot_test = sum((Y_test_norm_single - mean(Y_test_norm_single)).^2);
...
figure('Name','MLSSVR 개별 모델 (확장된 하이퍼파라미터) 10-fold 교차검증 결과','WindowStyle','docked');
bar(cv_scores); ylim([0 1]); grid on;
set(gca,'XTickLabel',output_names); ylabel('10-fold CV R²');
title('MLSSVR 개별 모델 (확장된 하이퍼파라미터) 10-fold 교차검증 성능'); xtickangle(45);
for j = 1:num_outputs
...
figure('Name','MLSSVR 개별 모델 (확장된 하이퍼파라미터) 예측 Correlation','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
...
sgtitle('MLSSVR 개별 모델 (확장된 하이퍼파라미터) 회귀 성능 : Correlation Plot','FontSize',14,'FontWeight','bold');

%% 2. Residual Plot (test)
figure('Name','MLSSVR 개별 모델 (확장된 하이퍼파라미터) Residual Plot (테스트셋)','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
...
sgtitle('MLSSVR 개별 모델 (확장된 하이퍼파라미터) 테스트셋 잔차 분석','FontSize',14);

%% 3. Boxplot 분포 비교 (train/test/예측)
figure('Name','MLSSVR 개별 모델 (확장된 하이퍼파라미터) 예측분포 Boxplot','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
...
sgtitle('MLSSVR 개별 모델 (확장된 하이퍼파라미터) train/test 분포비교','FontSize',14);

%% 4. 성능지표 막대그래프 (RMSE, MAE, R2)
figure('Name','MLSSVR 개별 모델 (확장된 하이퍼파라미터) 예측 성능지표(Bars)','WindowStyle','docked');
for k = 1:3
    subplot(1,3,k);
    if k==1
...
sgtitle('MLSSVR 개별 모델 (확장된 하이퍼파라미터) 성능지표 (output별)','FontSize',14);