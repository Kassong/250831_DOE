...
fprintf('\n=== Augment_2_MLSSVR_2G.m: 2개 그룹 MLSSVR 모델 예측 시작 ===\n');
fprintf('데이터 로딩 중...\n');
...
fprintf('최적 파라미터(G1): gamma=%.4f, lambda=%.4f, p=%.4f\n', gamma_opt_g1, lambda_opt_g1, p_opt_g1);
for i = 1:n_samples
...
fprintf('최적 파라미터(G2): gamma=%.4f, lambda=%.4f, p=%.4f\n', gamma_opt_g2, lambda_opt_g2, p_opt_g2);
for i = 1:n_samples
...
%% 6. LOOCV 결과 시각화
fprintf('\n=== LOOCV 결과 시각화 ===\n');
figure('Name','2-Group MLSSVR LOOCV 교차검증 결과','WindowStyle','docked');
bar(LOOCV_scores); ylim([0 1]); grid on;
set(gca,'XTickLabel',output_names); ylabel('LOOCV R²');
title('2-Group MLSSVR 모델 LOOCV 교차검증 성능'); xtickangle(45);
...
sgtitle('2-Group MLSSVR: 실제(16) vs 미실험(112) 예측 분포비교','FontSize',14,'FontWeight','bold');
...
sgtitle('2-Group MLSSVR 조건별 출력 예측 산점도 및 분포(16실제값 참조선)','FontSize',14,'FontWeight','bold');
...
sgtitle('2-Group MLSSVR 기반 예측값 히트맵','FontSize',14);

%% 8. 2-Group MLSSVR 예측값 CSV 저장
fprintf('\n=== 결과 저장 ===\n');
fprintf('2-Group MLSSVR 모델 예측값을 predict_MLSSVR_2G.csv로 저장 중...\n');
...
fprintf('저장 완료: predict_MLSSVR_2G.csv (%d개 예측조건)\n', size(X_predict,1));
fprintf('\n=== Augment_2_MLSSVR_2G.m 실행 완료 ===\n');