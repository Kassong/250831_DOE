...
fprintf('\n=== Augment_2_MLSSVR_1G_MSE.m: 단일 그룹 MLSSVR 모델 (그룹 MSE 최적화) 예측 시작 ===\n');
fprintf('데이터 로딩 중...\n');
...
%% 4. 그룹 MSE 기반 LOOCV 교차검증 (단일 그룹 MLSSVR)
fprintf('\n=== 단일 MLSSVR 모델(전체 출력 그룹) 그룹 MSE 기반 LOOCV 교차검증 시작 ===\n');
n_samples = size(X_train_norm, 1);

% MLSSVR 하이퍼파라미터 그리드 서치 (그룹 MSE 기반)
fprintf('그룹 MSE 기반 하이퍼파라미터 그리드 서치 중...\n');
[gamma_opt, lambda_opt, p_opt, MSE_opt] = GridMLSSVR_GroupMSE(X_train_norm, Y_train_norm, 5);
fprintf('최적 파라미터 (그룹 MSE=%.6f): gamma=%.4f, lambda=%.4f, p=%.4f\n', MSE_opt, gamma_opt, lambda_opt, p_opt);

% LOOCV 수행
fprintf('MLSSVR 그룹 MSE 기반 LOOCV 검증 중...\n');
...
for i = 1:n_samples
    ...
    % 그룹 MSE 계산
    loocv_mse = loocv_mse + mean((Y_loo_test - pred_loo).^2);
end
...
group_mse = loocv_mse / n_samples;
fprintf('전체 그룹 LOOCV MSE = %.6f\n', group_mse);
fprintf('MLSSVR 그룹 MSE 기반 LOOCV 교차검증 완료!\n');
...
%% 7. LOOCV 결과 시각화
fprintf('\n=== LOOCV 결과 시각화 ===\n');
figure('Name','단일 MLSSVR (그룹 MSE 최적화) LOOCV 교차검증 결과','WindowStyle','docked');
subplot(1,2,1);
bar(LOOCV_scores); ylim([0 1]); grid on;
set(gca,'XTickLabel',output_names); ylabel('LOOCV R²');
title('단일 MLSSVR (그룹 MSE 최적화) LOOCV R²'); xtickangle(45);
for j = 1:num_outputs
...
subplot(1,2,2);
bar(group_mse); grid on;
title(sprintf('단일 MLSSVR 그룹 LOOCV MSE: %.6f', group_mse));
ylabel('그룹 MSE'); xlabel('전체 그룹');

%% 8. 예측값 시각화
fprintf('\n=== 시각화 생성 ===\n');
% 8-1. 박스플롯: MLSSVR 예측 vs 실제
fprintf('박스플롯 생성 중...\n');
figure('Name','MLSSVR (그룹 MSE 최적화) 예측 분포 Boxplot: 미실험(112) vs 실제(16)','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
    data_box = [Y_train(:,j); Y_predict(:,j)];
    labels_box = [repmat({'실제 (16)'},size(Y_train,1),1); repmat({'MLSSVR 그룹MSE예측 (112)'},size(Y_predict,1),1)];
    boxplot(data_box, labels_box);
    title(['Output ',output_names{j}]); ylabel(output_names{j}); grid on;
end
sgtitle('단일 MLSSVR (그룹 MSE 최적화): 실제(16) vs 미실험(112) 예측 분포비교','FontSize',14,'FontWeight','bold');

% 8-2. 산점도: MLSSVR 예측조건별 실제값범위와 예측값
fprintf('산점도 및 히스토그램 생성 중...\n');
figure('Name','MLSSVR (그룹 MSE 최적화) 예측 산점도 및 히스토그램: 조건별 예측값 분포','WindowStyle','docked');
for j = 1:num_outputs
    subplot(4,2,j);
    scatter(1:size(Y_predict,1), Y_predict(:,j), 60, [0 0.7 0.3], 'filled'); hold on;
    yline(min(Y_train(:,j)),'k:','최소실제'); yline(max(Y_train(:,j)),'k:','최대실제');
    grid on; ylabel(output_names{j}); legend('MLSSVR 그룹MSE예측','location','best');
    title(['미실험 112조건 ',output_names{j},' 그룹MSE최적화 예측분포']);

    subplot(4,2,j+num_outputs);
    histogram(Y_predict(:,j),'FaceColor',[0 0.7 0.3],'EdgeAlpha',0.1); hold on;
    xline(min(Y_train(:,j)),'k:','최소실제'); xline(max(Y_train(:,j)),'k:','최대실제');
    grid on; xlabel(output_names{j}); ylabel('count'); legend('MLSSVR 그룹MSE예측','location','best');
    title(['미실험 112조건 ',output_names{j},' 그룹MSE최적화 예측 히스토그램']);
end
sgtitle('단일 MLSSVR (그룹 MSE 최적화) 조건별 출력 예측 산점도 및 분포(16실제값 참조선)','FontSize',14,'FontWeight','bold');

% 8-3. 히트맵 시각화(MLSSVR)
fprintf('히트맵 생성 중...\n');
figure('Name','MLSSVR (그룹 MSE 최적화) 예측값 히트맵','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
    Ymat = reshape(Y_predict(:,j), [14, 8]);
    imagesc(Ymat); colorbar;
    title(['Output ',output_names{j},' 그룹MSE최적화 예측 히트맵']);
    xlabel('X3*X4 조건 인덱스'); ylabel('X1*X2 조건 인덱스');
end
sgtitle('단일 MLSSVR (그룹 MSE 최적화) 기반 예측값 히트맵','FontSize',14);

%% 9. 단일 그룹 MLSSVR 예측값 CSV 저장
fprintf('\n=== 결과 저장 ===\n');
fprintf('단일 그룹 MLSSVR (그룹 MSE 최적화) 모델 예측값을 predict_MLSSVR_1G_MSE.csv로 저장 중...\n');
% MLSSVR 예측값 저장
predict_table = array2table([X_predict, Y_predict], ...
    'VariableNames',[input_names, output_names]);
writetable(predict_table, 'predict_MLSSVR_1G_MSE.csv');
fprintf('저장 완료: predict_MLSSVR_1G_MSE.csv (%d개 예측조건)\n', size(X_predict,1));
fprintf('\n=== Augment_2_MLSSVR_1G_MSE.m 실행 완료 ===\n');