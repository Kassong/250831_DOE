...
fprintf('\n=== Augment_2_MLSSVR_2G_MAE.m: 2개 그룹 MLSSVR 모델 (그룹별 MAE 최적화) 예측 시작 ===\n');
fprintf('데이터 로딩 중...\n');
...
%% 3. 그룹별 MAE 기반 하이퍼파라미터 최적화 함수 정의
...
%% 4. 그룹별 LOOCV 교차검증 (그룹별 MAE 최적화)
fprintf('\n=== 그룹별 MLSSVR 모델 그룹별 MAE 기반 LOOCV 교차검증 시작 ===\n');
...
% --- 그룹 1 (출력 1,2) MAE 최적화 ---
fprintf('\n--- 그룹 1 (출력 1,2) 그룹 MAE 기반 검증 중 ---\n');
fprintf('그룹 1 MAE 기반 하이퍼파라미터 그리드 서치 중...\n');
[gamma_opt_g1, lambda_opt_g1, p_opt_g1, mae_opt_g1] = GridMLSSVR_GroupMAE(X_train_norm, Y_train_norm_g1, 5);
fprintf('최적 파라미터(G1, 그룹 MAE=%.4f): gamma=%.4f, lambda=%.4f, p=%.4f\n', mae_opt_g1, gamma_opt_g1, lambda_opt_g1, p_opt_g1);
...
for i = 1:n_samples
    ...
    loocv_mae_g1 = loocv_mae_g1 + mean(abs(Y_loo_test - pred_loo));
end

% --- 그룹 2 (출력 3,4) MAE 최적화 ---
fprintf('\n--- 그룹 2 (출력 3,4) 그룹 MAE 기반 검증 중 ---\n');
fprintf('그룹 2 MAE 기반 하이퍼파라미터 그리드 서치 중...\n');
[gamma_opt_g2, lambda_opt_g2, p_opt_g2, mae_opt_g2] = GridMLSSVR_GroupMAE(X_train_norm, Y_train_norm_g2, 5);
fprintf('최적 파라미터(G2, 그룹 MAE=%.4f): gamma=%.4f, lambda=%.4f, p=%.4f\n', mae_opt_g2, gamma_opt_g2, lambda_opt_g2, p_opt_g2);
...
for i = 1:n_samples
    ...
    loocv_mae_g2 = loocv_mae_g2 + mean(abs(Y_loo_test - pred_loo));
end
...
MAE_scores = [loocv_mae_g1/n_samples, loocv_mae_g2/n_samples];

for j=1:length(output_names), fprintf('  %s LOOCV R² = %.4f\n', output_names{j}, LOOCV_scores(j)); end
fprintf('그룹 1 LOOCV MAE = %.4f, 그룹 2 LOOCV MAE = %.4f\n', MAE_scores(1), MAE_scores(2));
fprintf('MLSSVR 그룹별 MAE 기반 LOOCV 교차검증 완료!\n');
...
%% 7. LOOCV 결과 시각화
fprintf('\n=== LOOCV 결과 시각화 ===\n');
figure('Name','2-Group MLSSVR (그룹별 MAE 최적화) LOOCV 교차검증 결과','WindowStyle','docked');
subplot(1,2,1);
bar(LOOCV_scores); ylim([0 1]); grid on;
set(gca,'XTickLabel',output_names); ylabel('LOOCV R²');
title('2-Group MLSSVR (그룹별 MAE 최적화) LOOCV R²'); xtickangle(45);
...
set(gca,'XTickLabel',{'그룹1 (Y1,Y2)', '그룹2 (Y3,Y4)'}); ylabel('그룹 LOOCV MAE');
title('2-Group MLSSVR 그룹별 LOOCV MAE'); xtickangle(45);
for j = 1:length(MAE_scores)
...
fprintf('\n=== 시각화 생성 ===\n');
% 8-1. 박스플롯
figure('Name','MLSSVR (그룹별 MAE 최적화) 예측 분포 Boxplot: 미실험(112) vs 실제(16)','WindowStyle','docked');
for j = 1:length(output_names)
    subplot(2,2,j);
    data_box = [Y_train(:,j); Y_predict(:,j)];
    labels_box = [repmat({'실제 (16)'},size(Y_train,1),1); repmat({'MLSSVR 그룹별MAE예측 (112)'},size(Y_predict,1),1)];
    boxplot(data_box, labels_box);
    title(['Output ',output_names{j}]); ylabel(output_names{j}); grid on;
end
sgtitle('2-Group MLSSVR (그룹별 MAE 최적화): 실제(16) vs 미실험(112) 예측 분포비교','FontSize',14,'FontWeight','bold');

% 8-2. 산점도 및 히스토그램
figure('Name','MLSSVR (그룹별 MAE 최적화) 예측 산점도 및 히스토그램: 조건별 예측값 분포','WindowStyle','docked');
for j = 1:length(output_names)
    subplot(4,2,j);
    scatter(1:size(Y_predict,1), Y_predict(:,j), 60, 'm', 'filled'); hold on;
    yline(min(Y_train(:,j)),'k:','최소실제'); yline(max(Y_train(:,j)),'k:','최대실제');
    grid on; ylabel(output_names{j}); legend('MLSSVR 그룹별MAE예측','location','best');
    title(['미실험 112조건 ',output_names{j},' 그룹별MAE최적화 예측분포']);
    
    subplot(4,2,j+length(output_names));
    histogram(Y_predict(:,j),'FaceColor','m','EdgeAlpha',0.1); hold on;
    xline(min(Y_train(:,j)),'k:','최소실제'); xline(max(Y_train(:,j)),'k:','최대실제');
    grid on; xlabel(output_names{j}); ylabel('count'); legend('MLSSVR 그룹별MAE예측','location','best');
    title(['미실험 112조건 ',output_names{j},' 그룹별MAE최적화 예측 히스토그램']);
end
sgtitle('2-Group MLSSVR (그룹별 MAE 최적화) 조건별 출력 예측 산점도 및 분포(16실제값 참조선)','FontSize',14,'FontWeight','bold');

% 8-3. 히트맵
figure('Name','MLSSVR (그룹별 MAE 최적화) 예측값 히트맵','WindowStyle','docked');
for j = 1:length(output_names)
    subplot(2,2,j);
    Ymat = reshape(Y_predict(:,j), [14, 8]);
    imagesc(Ymat); colorbar;
    title(['Output ',output_names{j},' 그룹별MAE최적화 예측 히트맵']);
    xlabel('X3*X4 조건 인덱스'); ylabel('X1*X2 조건 인덱스');
end
sgtitle('2-Group MLSSVR (그룹별 MAE 최적화) 기반 예측값 히트맵','FontSize',14);

%% 9. 2-Group MLSSVR 예측값 CSV 저장
fprintf('\n=== 결과 저장 ===\n');
fprintf('2-Group MLSSVR (그룹별 MAE 최적화) 모델 예측값을 predict_MLSSVR_2G_MAE.csv로 저장 중...\n');
predict_table = array2table([X_predict, Y_predict], ...
    'VariableNames',[input_names, output_names]);
writetable(predict_table, 'predict_MLSSVR_2G_MAE.csv');
fprintf('저장 완료: predict_MLSSVR_2G_MAE.csv (%d개 예측조건)\n', size(X_predict,1));
fprintf('\n=== Augment_2_MLSSVR_2G_MAE.m 실행 완료 ===\n');