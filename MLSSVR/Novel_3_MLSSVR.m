clear; clc; close all;
rng('default'); set(0, 'DefaultFigureWindowStyle', 'docked');
addpath('MLSSVR-master');

fprintf('\n=== 개별 MLSSVR 모델 학습/평가 시작 ===\n');
fprintf('학습데이터: predict_MLSSVR.csv, 테스트데이터: dataset.csv\n');
T_train = readtable('predict_MLSSVR.csv');      % 112개
T_test  = readtable('dataset.csv');      % 16개

X_train = T_train{:,1:4}; Y_train = T_train{:,5:8};
X_test  = T_test{:,1:4};  Y_test  = T_test{:,5:8};

input_names = T_train.Properties.VariableNames(1:4);
output_names = T_train.Properties.VariableNames(5:8);
num_outputs = size(Y_train,2);

fprintf('데이터셋 로드 완료: 학습 %d건, 테스트 %d건\n', size(X_train,1),size(X_test,1));

% 정규화(학습셋 기준)
[X_train_norm, X_mean, X_std] = zscore(X_train);
X_test_norm = (X_test - X_mean) ./ X_std;
[Y_train_norm, Y_mean, Y_std] = zscore(Y_train);
Y_test_norm = (Y_test - Y_mean) ./ Y_std;

% 최종 결과를 저장할 변수들 초기화
Y_pred_train_norm = zeros(size(Y_train_norm));
Y_pred_test_norm = zeros(size(Y_test_norm));
cv_scores = zeros(num_outputs, 1);
RMSE_train = zeros(num_outputs,1); RMSE_test = zeros(num_outputs,1);
MAE_train  = zeros(num_outputs,1); MAE_test  = zeros(num_outputs,1);
R2_train   = zeros(num_outputs,1); R2_test   = zeros(num_outputs,1);

%% 각 출력 변수별로 개별 모델 학습 및 평가 루프
for j = 1:num_outputs
    fprintf('\n====== 출력 변수 %s (%d/%d) 모델링 시작 ======\n', output_names{j}, j, num_outputs);

    % 현재 출력 변수에 해당하는 데이터만 선택
    Y_train_norm_single = Y_train_norm(:, j);
    Y_test_norm_single  = Y_test_norm(:, j);

    % --- 10-fold 교차검증 (개별 모델) ---
    fprintf('10-fold 교차검증 시작...\n');
    k_folds = 10;
    n_train = size(X_train_norm, 1);

    % 하이퍼파라미터 서치 (현재 출력 변수 데이터만 사용)
    [gamma_opt, lambda_opt, p_opt, ~] = GridMLSSVR(X_train_norm, Y_train_norm_single, 5);
    fprintf('최적 파라미터: gamma=%.4f, lambda=%.4f, p=%.4f\n', gamma_opt, lambda_opt, p_opt);

    cv_indices = cvpartition(n_train, 'KFold', k_folds);
    cv_pred = zeros(n_train, 1);
    for fold = 1:k_folds
        train_idx = training(cv_indices, fold);
        test_idx = test(cv_indices, fold);
        
        X_cv_train = X_train_norm(train_idx, :);
        Y_cv_train_single = Y_train_norm_single(train_idx, :);
        X_cv_test = X_train_norm(test_idx, :);
        
        [alpha_cv, b_cv] = MLSSVRTrain(X_cv_train, Y_cv_train_single, gamma_opt, lambda_opt, p_opt);
        
        Y_cv_test_dummy = zeros(sum(test_idx), 1);
        [pred_cv, ~, ~] = MLSSVRPredict(X_cv_test, Y_cv_test_dummy, X_cv_train, alpha_cv, b_cv, lambda_opt, p_opt);
        cv_pred(test_idx) = pred_cv;
    end
    
    SS_res_cv = sum((Y_train_norm_single - cv_pred).^2);
    SS_tot_cv = sum((Y_train_norm_single - mean(Y_train_norm_single)).^2);
    cv_scores(j) = 1 - SS_res_cv / SS_tot_cv;
    fprintf('10-fold CV R² = %.4f\n', cv_scores(j));

    % --- 최종 개별 모델 학습 및 예측 ---
    fprintf('최종 모델 학습 및 예측...\n');
    [alpha, b] = MLSSVRTrain(X_train_norm, Y_train_norm_single, gamma_opt, lambda_opt, p_opt);

    Y_train_dummy = zeros(size(Y_train_norm_single));
    [Y_pred_train_norm(:, j), ~, ~] = MLSSVRPredict(X_train_norm, Y_train_dummy, X_train_norm, alpha, b, lambda_opt, p_opt);
    
    Y_test_dummy = zeros(size(Y_test_norm_single));
    [Y_pred_test_norm(:, j), ~, ~] = MLSSVRPredict(X_test_norm, Y_test_dummy, X_train_norm, alpha, b, lambda_opt, p_opt);

    % --- 성능 지표 계산 ---
    RMSE_train(j) = sqrt(mean((Y_train_norm_single - Y_pred_train_norm(:, j)).^2));
    RMSE_test(j)  = sqrt(mean((Y_test_norm_single - Y_pred_test_norm(:, j)).^2));
    MAE_train(j)  = mean(abs(Y_train_norm_single - Y_pred_train_norm(:, j)));
    MAE_test(j)   = mean(abs(Y_test_norm_single - Y_pred_test_norm(:, j)));
    
    SS_res_train = sum((Y_train_norm_single - Y_pred_train_norm(:, j)).^2);
    SS_tot_train = sum((Y_train_norm_single - mean(Y_train_norm_single)).^2);
    R2_train(j) = 1 - SS_res_train/SS_tot_train;
    
    SS_res_test = sum((Y_test_norm_single - Y_pred_test_norm(:, j)).^2);
    SS_tot_test = sum((Y_test_norm_single - mean(Y_test_norm_single)).^2);
    R2_test(j) = 1 - SS_res_test/SS_tot_test;
    
    fprintf('%s: Train R²=%.3f, Test R²=%.3f\n', output_names{j}, R2_train(j), R2_test(j));
end

%% 역정규화
Y_pred_train = Y_pred_train_norm .* Y_std + Y_mean;
Y_pred_test  = Y_pred_test_norm  .* Y_std + Y_mean;

%% 전체 지표 테이블 출력
fprintf('\n====== 최종 성능 요약 (개별 모델) ======\n');
metrics_table = table(output_names', cv_scores, RMSE_train, RMSE_test, MAE_train, MAE_test, R2_train, R2_test, ...
    'VariableNames', {'Output','CV_R2','RMSE_train','RMSE_test','MAE_train','MAE_test','R2_train','R2_test'});
disp(metrics_table);

%% 10-fold CV 결과 시각화
fprintf('\n=== MLSSVR 10-fold CV 결과 시각화 ===\n');
figure('Name','MLSSVR 개별 모델 10-fold 교차검증 결과','WindowStyle','docked');
bar(cv_scores); ylim([0 1]); grid on;
set(gca,'XTickLabel',output_names); ylabel('10-fold CV R²');
title('MLSSVR 개별 모델 10-fold 교차검증 성능'); xtickangle(45);
for j = 1:num_outputs
    text(j, cv_scores(j)+0.05, sprintf('%.3f', cv_scores(j)), ...
        'HorizontalAlignment', 'center', 'FontWeight', 'bold');
end

%% 1. 예측값 vs 실제값 산점도 플롯 (train/test)
figure('Name','MLSSVR 개별 모델 예측 Correlation','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
    scatter(Y_pred_test(:,j), Y_test(:,j), 120,'r','filled'); hold on;
    scatter(Y_pred_train(:,j), Y_train(:,j),90,'b','filled','MarkerFaceAlpha',0.6);
    xls = min([Y_pred_test(:,j); Y_test(:,j); Y_pred_train(:,j); Y_train(:,j)]);
    xhs = max([Y_pred_test(:,j); Y_test(:,j); Y_pred_train(:,j); Y_train(:,j)]);
    plot([xls xhs],[xls xhs],'k--','LineWidth',1.2);
    grid on; axis equal; box on; legend('테스트','학습','1:1 참조','Location','best');
    xlabel('예측값'); ylabel('실제값');
    title(sprintf('%s\nRMSE_train=%.3f, test=%.3f\nMAE_train=%.3f, test=%.3f\nR²_train=%.3f, test=%.3f', ...
        output_names{j}, RMSE_train(j), RMSE_test(j), MAE_train(j), MAE_test(j), R2_train(j), R2_test(j)));
end
sgtitle('MLSSVR 개별 모델 회귀 성능 : Correlation Plot','FontSize',14,'FontWeight','bold');

%% 2. Residual Plot (test)
figure('Name','MLSSVR 개별 모델 Residual Plot (테스트셋)','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
    residual = Y_test(:,j) - Y_pred_test(:,j);
    scatter(Y_pred_test(:,j), residual, 100,'filled');
    yline(0,'k--');
    grid on; xlabel('테스트 예측값'); ylabel('잔차(실제-예측)');
    title(['Output ',output_names{j},' residual']);
end
sgtitle('MLSSVR 개별 모델 테스트셋 잔차 분석','FontSize',14);

%% 3. Boxplot 분포 비교 (train/test/예측)
figure('Name','MLSSVR 개별 모델 예측분포 Boxplot','WindowStyle','docked');
for j = 1:num_outputs
    subplot(2,2,j);
    data_box = [Y_train(:,j); Y_pred_train(:,j); Y_test(:,j); Y_pred_test(:,j)];
    labels_box = [repmat({'Train_실제'},size(Y_train,1),1); repmat({'Train_예측'},size(Y_pred_train,1),1); ...
                  repmat({'Test_실제'},size(Y_test,1),1); repmat({'Test_예측'},size(Y_pred_test,1),1)];
    boxplot(data_box, labels_box);
    grid on; ylabel(output_names{j});
    title(['Output ',output_names{j},' 분포비교']);
end
sgtitle('MLSSVR 개별 모델 train/test 분포비교','FontSize',14);

%% 4. 성능지표 막대그래프 (RMSE, MAE, R2)
figure('Name','MLSSVR 개별 모델 예측 성능지표(Bars)','WindowStyle','docked');
for k = 1:3
    subplot(1,3,k);
    if k==1
        bar_data = [RMSE_train, RMSE_test];
        title_text = 'RMSE 비교';
        ylabel_text = 'RMSE';
    elseif k==2
        bar_data = [MAE_train, MAE_test];
        title_text = 'MAE 비교';
        ylabel_text = 'MAE';
    else
        bar_data = [R2_train, R2_test];
        title_text = 'R² 비교';
        ylabel_text = 'R²';
    end
    bar(bar_data, 'grouped');
    set(gca,'xticklabel',output_names);
    legend('Train','Test','Location','best'); 
    ylabel(ylabel_text);
    title(title_text);
    grid on;
end
sgtitle('MLSSVR 개별 모델 성능지표 (output별)','FontSize',14);